 ...
 * @LastEditors: gitsrc
 * @LastEditTime: 2020-08-05 18:27:37
 * @FilePath: /redis-cluster-proxy/utils/rediscluster/cluster.go
 */
...
func (cluster *Cluster) Do(cmd string, args ...interface{}) (interface{}, error) {
	...
	var reply interface{}
	if cmd == "RPUSH" {
		reply, err = node.doRpushExpire(cmd, args...)
	} else {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("Do->node.do(%s) %v", cmd, err)
	}
...
func (cluster *Cluster) SlaveDo(cmd string, args ...interface{}) (interface{}, error) {
	...
	if err != nil {
		...
		node, err = cluster.getNodeByKey(args[0])
		if err != nil {
			return nil, fmt.Errorf("SlaveDo->cluster.getNodeByKey() %v", err)
		}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("SlaveDo->node.getConn() %v", err)
	}
	...
	case "GET": //如果是从节点，则发送READONLY指令
		if node.NodeType == SLAVE_NODE {
			...
			if err != nil {
				conn.shutdown()
				return nil, fmt.Errorf("SlaveDo->node.send->READONLY() %v", err)
			}
		}
	}
	...
	if err != nil {
		conn.shutdown()
		//这里的err不为空，一定是网络读取错误，但是一条TCP出错不应该直接调用槽位更新: 可能是此条TCP出现问题
		return nil, fmt.Errorf("SlaveDo->conn.send(%s) %v", cmd, err)
	}
	...
	if err != nil {
		conn.shutdown()
		return nil, fmt.Errorf("SlaveDo->conn.flush(%s) %v", cmd, err)
	}
	...
	case "GET": //如果是从节点，则处理前面的READONLY指令响应
		if node.NodeType == SLAVE_NODE {
			...
			if err != nil {
				conn.shutdown()
				return nil, fmt.Errorf("SlaveDo->node.receive->READONLY() %v", err)
			}
...
func (cluster *Cluster) GetNodeAddrByKey(arg interface{}) (string, error) {
	key, err := key(arg)
	if err != nil {
		return "", fmt.Errorf("getNodeByKey: invalid key %v", key)
	}
	...
	if cluster.closed {
		return "", fmt.Errorf("getNodeByKey: cluster has been closed")
	}

	node := cluster.slots[slot]
	if node == nil {
		return "", fmt.Errorf("getNodeByKey: %s[%d] no node found", key, slot)
	}
