...
func (node *redisNode) doRpushExpire(cmd string, args ...interface{}) (interface{}, error) {
	conn, err := node.getConn()
	if err != nil {
		return redisError("ECONNTIMEOUT"), nil
	}

	if err = conn.send(cmd, args[0], args[1]); err != nil {
		conn.shutdown()
		return nil, err
	}
	...
	if err = conn.send("expire", args[0], expire); err != nil {
		conn.shutdown()
		return nil, err
	}
